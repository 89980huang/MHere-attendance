...
class StudentAddCourseViewController: UIViewController,
    ...
    func updateCourseList() {
        getAllCourses { () -> Void in
            ...
            self.coursesOptions = Array(set1.subtracting(set2))
            ...
            self.coursesOptions = self.coursesOptions.sorted()
    ...
    func getAllCourses(_ callback: () -> Void ) {
    ...
    @IBAction func addCourseButtonPressed(_ sender: AnyObject) {
        ...
        if currentCourses.index(where: { $0 == selectedKey }) != nil {
            Utils.endHUD(false)
