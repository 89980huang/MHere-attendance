...
struct Course {
    ...
    var room: String = ""
    var durationStart: String = Foundation.Date().stringFromDate(Foundation.Date(), format: Config.dateFormatInServer)
    var durationEnd: String = Foundation.Date().stringFromDate(Foundation.Date(), format: Config.dateFormatInServer)
    var timeStart: String = Foundation.Date().stringFromDate(Foundation.Date(), format: Config.dateFormatInServer)
    var timeEnd: String = Foundation.Date().stringFromDate(Foundation.Date(), format: Config.dateFormatInServer)
    var trimester: String = "Spring 2016"
    var dayOfWeek: Int? = nil
    var selectedDates:[Foundation.Date]? = nil
    ...
    func getStringSelectedDates() -> [String] {
        ...
        let stringDates = self.selectedDates?.map({ date in
            Foundation.Date().stringFromDate(date, format: Config.dateFormatInServer)
        })
    ...
    static func JSONtoCourse(_ json: JSON, isWeekend: Bool? = nil) -> Course {
        /* weekend
        "course_section" : "1",
        ...
        */
        ...
        if let dayOfWeek = dayOfWeek, selectedDates.isEmpty {
            ...
            newCourse.selectedDates = selectedDates.map({ Foundation.Date().dateFromString($0.stringValue, format: Config.dateFormatInServer) })
