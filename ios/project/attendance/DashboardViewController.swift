...
class DashboardViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        dateLabel.text = Foundation.Date().stringFromDate(Foundation.Date(), format: "EEEE, MMMM d")
    ...
    func updateUserInfo() {
        ...
        guard let role = self.userDefaults.string(forKey: UDKeys.userRole) else {
            log.error("Token missing. Cannot make request.")
            return
        }
        
        guard let token = self.userDefaults.string(forKey: UDKeys.token) else {
            log.error("Token missing. Cannot make request.")
            return
        }
...
//                            let courses = json["courses"].arrayValue
    ...
    func prepareBarModels(dataSource: [(String, ( Int , Int ) )]) -> [ChartBarModel] {
...
//            print(data.1, data.1.0, data.1.1)
    ...
    fileprivate func barsChart(horizontal: Bool, barModels: [ChartBarModel], xLabelTitle: String? = "Courses") -> Chart {
        let labelSettings = ChartLabelSettings(font: ChartDefaults.labelFont)
        
        let (axisValues1, axisValues2) = (
            stride(from: 0, through: 100, by: 20).map {ChartAxisValueFloat(CGFloat($0), labelSettings: labelSettings)},
...
//        print(viewHeight)
        let frame = CGRect(x: 0, y: 220, width: viewWidth, height: viewHeight - 220 )
    ...
    fileprivate func showChart(horizontal: Bool, barModels: [ChartBarModel], xLabelTitle: String? = "Courses") {
    ...
    func prepareTabForRole(_ role:String) {
