...
class TakeManualAttendanceTableViewController: UITableViewController {
    ...
    @IBAction func addAttendancePressed(_ sender: AnyObject) {
        ...
        requester.updateManualAttendance(email, courseKey: courseKey, dateTime: Foundation.Date().stringFromDate(datePicker.date, format: Config.dateFormatInServer), failureCallback: { (response) -> Void in
